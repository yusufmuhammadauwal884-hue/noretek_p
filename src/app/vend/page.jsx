...
export default function CustomerTokenPage() {
  ...
  // Fetch customer accounts with improved error handling
  const fetchCustomerAccounts = async (token) => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/API/Account/Read`, {
        method: "POST",
        headers: {
      ...
      if (data.result && data.result.data) {
        ...
        setMessage("ℹ️ No customer accounts found");
        setMessageType("info");
      }
    } catch (error) {
  ...
  // Fetch tokens for a specific customer with improved error handling
  const fetchCustomerTokens = async (customerId, meterId, customer) => {
    setLoading(true);
    setSelectedCustomer(customer);
    
    try {
      const response = await fetch(`${API_BASE_URL}/API/Token/CreditTokenRecord/Read`, {
        method: "POST",
        headers: {
      ...
      if (data.result && data.result.data) {
        ...
        setMessage("ℹ️ No tokens found for this customer");
        setMessageType("info");
      }
    } catch (error) {
  ...
  // Generate token for a customer with improved error handling
  const generateToken = async (e) => {
    ...
    try {
      ...
      const totalUnit = amount / currentPricePerKg;

      const response = await fetch(`${API_BASE_URL}/API/Token/CreditToken/Generate`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${authToken}`,
          "Content-Type": "application/json",
  ...
  const handleLogout = () => {
    ...
    setCustomers([]);
    setCustomerTokens([]);
    setMessage("ℹ️ You have been logged out");
    setMessageType("info");
    setSearchTerm("");
  };
        ...
        <div className="container">
          ...
          {isAuthenticated && (
            <>
              {/* Message Display */}
              {message && (
                <div className={`alert alert-${messageType === "success" ? "success" : messageType === "info" ? "info" : "danger"} mb-4`}>
                  {message}
                </div>
              )}
              ...
              <div className="card shadow">
                <div className="card-header primaryColor d-flex justify-content-between align-items-center">
                  <h5 className="mb-0 text-white"><i className="fas fa-list-alt me-2"></i>Registered Customer Accounts</h5>
                  <button 
            ...
            <div className="modal fade show d-block" style={{backgroundColor: 'rgba(0,0,0,0.5)'}}>
              <div className="modal-dialog modal-lg">
                <div className="modal-content">
                  <div className="modal-header primaryColor text-white">
